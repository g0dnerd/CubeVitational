...
class Tournament(object):
    def __init__(self, startingTable=1):
        ...
        # this defines the max number of players in a network point range before we split it up. Lower the number,
    ...
    def add_player(self, IDNumber, playerName, fixedSeating=False):

        """
        Holds player data that are in the event.

        Each player entry is a dictionary named by ID#

        ID : { Name:String,
                Opponents:List, Each entry is an ID number of someone you played
    ...
    def load_event_data(self, pathToLoad):
        self.playersDict = pickle.load(open(pathToLoad, "rb"))

    def save_event_data(self, pathToSave):
        ...
        if not len(self.tablesOut) or forcePair:
            ...
            for points in pointTotals:
                ...
                for player in bracketGraph.nodes():
                    for opponent in bracketGraph.nodes():
                        if opponent not in self.playersDict[player]["Opponents"] and player != opponent:
                            ...
                            if self.playersDict[player]["Points"] > int(points.split('_')[0]) or \
                                    self.playersDict[opponent]["Points"] > int(points.split('_')[0]):
                                wgt = 10
                            # Create edge
                ...
                if len(pointLists[points]) > 0:
                    ...
                    printdbg("Player %s left in %s. The index is %s and the length of totals is %s" % (
                    ...
                    if pointTotals.index(points) + 1 == len(pointTotals):
                        while len(pointLists[points]) > 0:
                            ...
                            self.assign_bye(pointLists[points].pop(0))
                    else:
            ...
            for match in displacedMatches:
                if len(openTables):
                    ...
                    self.pair_players(match[0], match[1])
    ...
    def report_match(self, table, result):
        ...
        if result[0] == result[1]:
            # If values are the same, players drew - give them each a point
            ...
            # Figure out who won and adding points
            if result[0] > result[1]:
        ...
        # When the last table reports, update tiebreakers automatically
        ...
        for p in self.playersDict:
            ...
            for opponent in self.playersDict[p]["Opponents"]:
                ...
                if opponent != "bye":
                    ...
                    winPercent = max(self.playersDict[opponent]["Points"] / float(
                        (len(self.playersDict[opponent]["Opponents"]) * 3)), 0.33)
...
def printdbg(msg, level=1):
    if dbg is True and level <= debuglevel:
        print(msg)
